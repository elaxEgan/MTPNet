...
class saliency_prompt_inference(nn.Module):
    def __init__(self, dim, num_heads=1, qkv_bias=False, qk_scale=None, attn_drop=0., proj_drop=0.):
        super().__init__()
    ...
    def forward(self, fea, task_prompt, num):
        B, N, C = fea.shape
        x = self.norm(fea)
        ...
        k = self.k(task_prompt).reshape(B, num, self.num_heads, C // self.num_heads).permute(0, 2, 1, 3)
        v = self.v(task_prompt).reshape(B, num, self.num_heads, C // self.num_heads).permute(0, 2, 1, 3)
        ...
        infer_fea = infer_fea + fea
        return infer_fea


class edge_prompt_inference(nn.Module):
    def __init__(self, dim, num_heads=1, qkv_bias=False, qk_scale=None, attn_drop=0., proj_drop=0.):
        super().__init__()
...
class token_Transformer(nn.Module):
    def __init__(self, embed_dim=384, depth=14, num_heads=6, mlp_ratio=3., in_dim=64):
        ...
        self.saliency_prompt_pre = saliency_prompt_inference(dim=embed_dim, num_heads=1)
        self.edge_prompt_pre = edge_prompt_inference(dim=embed_dim, num_heads=1)
    ...
    def forward(self, rgb_fea, num, task_prompt1_d):
        B, _, _ = rgb_fea.shape
        ...
        fea_1_16, saliency_tokens, edge_tokens, fea_16, fea_1_16_s, task_prompt = self.encoderlayer(fea_1_16, 2 * num)

        saliency_fea_1_16 = self.saliency_prompt_pre(fea_1_16, task_prompt[:, :num, :], num)
        edge_fea_1_16 = self.edge_prompt_pre(fea_1_16, task_prompt[:, num:, :], num)
        ...
        return fea_1_16, saliency_tokens, edge_tokens, fea_16, saliency_tokens_tmp, edge_tokens_tmp, saliency_fea_1_16, edge_fea_1_16, task_prompt
