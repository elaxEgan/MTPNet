...
class WindowAttention(nn.Module):
    ...
    def forward(self, x, num, mask=None):
        ...
        relative_position_bias = torch.cat(
            (prompt_PE.repeat((num), self.window_size[0] * self.window_size[1], 1), relative_position_bias),
            dim=0)
        relative_position_bias = torch.cat((prompt_PE.repeat(self.window_size[0] * self.window_size[1] + (num),
                                                             (num), 1), relative_position_bias), dim=1)
        ...
        if mask is not None:
            ...
            mask = torch.cat((prompt_mask.repeat(1, self.window_size[0] * self.window_size[1], (num)), mask),
                             dim=2)
            mask = torch.cat(
                (prompt_mask.repeat(1, (num), self.window_size[0] * self.window_size[1] + (num)), mask),
                dim=1)
...
class SwinTransformerBlock(nn.Module):
    ...
    def forward(self, x, task_prompt, num, ):
        H, W = self.input_resolution
        B, L, C = x.shape
        ...
        # W-MSA/SW-MSA
        attn_windows = self.attn(x_windows, 2 * num, mask=self.attn_mask)  # nW*B, window_size*window_size, C
        attn_windows = attn_windows[:, 2 * num:, :]
        prompts = attn_windows[:, :2 * num, :]
    ...
    def forward(self, x, task_prompt, num):
        for blk in self.blocks:
            if self.use_checkpoint:
                x = checkpoint.checkpoint(blk, x, task_prompt, num)
            else:
                x = blk(x, task_prompt, num)
...
class SwinTransformer(nn.Module):
    ...
    def forward_features(self, x, task_prompt, num):
        x = self.patch_embed(x)
        if self.ape:
            x = x + self.absolute_pos_embed
        x = self.pos_drop(x)

        x_1_4, x_1_8 = self.layers[0](x, task_prompt[0], num[0])
        x_1_8, x_1_16 = self.layers[1](x_1_8, task_prompt[1], num[1])
        x_1_16, x_1_32 = self.layers[2](x_1_16, task_prompt[2], num[2])
        x_1_32 = self.layers[3](x_1_32, task_prompt[3], num[3])
    ...
    def forward(self, x, task_prompt, num):
        x_1_4, x_1_8, x_1_16, x_1_32 = self.forward_features(x, task_prompt, num)
        x = self.head(x_1_32)
        return x_1_4, x_1_8, x_1_16, x_1_32
