...
model = MTPNet(opt)
model.load_state_dict(torch.load('./models/MTPNet/MTPNet_EORSSD.pth.best'))
...
for dataset in test_datasets:
    ...
    gt_root = dataset_path + dataset + '/labels/'
    test_loader = test_dataset(image_root, gt_root, opt.testsize, opt.batchsize)
    time_sum = 0
    ...
    for i in range(num_batches):
        ...
        task_prompt_total2,  task_prompt_total1, outputs_saliency, outputs_contour, outputs_saliency_s, outputs_contour_s, a= model(
            images)
        time_end = time.time()
