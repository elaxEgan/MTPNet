...
class test_dataset:
    def __init__(self, image_root, gt_root, testsize, batch_size=4):
    ...
    def load_data(self):
        ...
        for _ in range(self.batch_size):
            if self.index >= self.size:
                self.index = 0  # reset index if all images have been loaded
            image = self.rgb_loader(self.images[self.index])
            image = self.img_transform(image)  # remove unsqueeze here
